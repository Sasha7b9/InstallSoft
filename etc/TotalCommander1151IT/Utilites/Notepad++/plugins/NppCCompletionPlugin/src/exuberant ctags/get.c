 ...
 */
static boolean readIdentifier (int c, vString *const name)///
{
	vStringClear (name);
	do
	{
	...
	///naj ta funkcija vrne, ali je naslednji znak oklepaj (macro ne sme imeti presledkov pred oklepajem)
	boolean isFunctionLikeMacro = c == '(';///
	fileUngetc (c);
	vStringTerminate (name);
	return (isFunctionLikeMacro);///
}
...
static void makeDefineTag (const char *const name, const boolean isFunctionLikeMacro)///
{
		...
		(! isFileScope  ||  Option.include.fileScope))
	{
		...
		e.kindName     = "macro";
		e.kind         = 'd';
		e.isFunctionLikeMacro = isFunctionLikeMacro;///
		makeTagEntry (&e);
	}
}
...
static void directiveDefine (const int c)
{
	if (isident1 (c))
	{
		boolean isFunctionLikeMacro = readIdentifier (c, Cpp.directive.name);///
		if (! isIgnore ())
			makeDefineTag (vStringValue (Cpp.directive.name), isFunctionLikeMacro);///
	}
	Cpp.directive.state = DRCTV_NONE;
}
