...
static void addPseudoTags (void)
{
	if (! Option.xref)
	{
		...
		writePseudoTag ("TAG_PROGRAM_MOD_BY",  "Bostjan Mihoric",  "ccompletion@gmail.com");///
		writePseudoTag ("TAG_PROGRAM_NAME",    PROGRAM_NAME, "unofficial modification");///
		writePseudoTag ("TAG_PROGRAM_URL",     PROGRAM_URL,  "official site");
		writePseudoTag ("TAG_PROGRAM_VERSION", PROGRAM_VERSION, "unofficial modification");///
	}
}
...
#define sep (first ? (first = FALSE, separator) : empty)
	...
	if (tag->isFunctionLikeMacro)///nisem dodajal extension field-a, ker to ni nek release za splošno uporabo
		length += fprintf (TagFile.fp, "%s\tfunlike:", sep);///
